...
export function RequestTagStatistics(tag) {
    return (dispatch) => {
        ...
        try{
            ...
            StatsService.getPopularAndScore(tag).then((data)=> {

                dispatch(ChangeYoutubePopular(data.topFiveTags));

                StatsService.getScore(tag, data.totalResults).then(data => {
                    dispatch(ChangeTagScore(data));
                });
            });

            //then doing trends and autocomplete stuff
            TrendsService.getTrends(tag).then((data) =>{
                dispatch(ChangeTrends(data));
            });

            AutoService.searchYandexAuto(tag).then((data) => {
                dispatch(ChangeYandexAuto(data));
            });

            AutoService.searchGoogleAuto(tag).then((data)=> {
                dispatch(ChangeGoogleAuto(data));
            });

            AutoService.searchYoutubeAuto(tag).then((data)=> {
